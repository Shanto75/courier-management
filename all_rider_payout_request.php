...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['parcelid'])) {
    ...
    $sql = "UPDATE rider_payout set billid = '$billidnumber' where parcelref = '$ref' ";
...
<div class="modal fade mx-auto" id="addbillid" tabindex="-1" role="dialog" aria-labelledby="editModalLabel"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form class=" mx-auto p-5" action="index.php?page=all_rider_payout_request" method="post">
...
<div class="card card-outline card-primary">
    <div class="card-header">
        ...
        <div class="">
            <div class="row">
                ...
                <div class="col-md-2">
                    <a class="btn btn-sm btn-default btn-flat border-primary " href="./index.php?page=new_payment"><i
                            class="fa fa-money-bill"></i> Request For Payment</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card-body">
        <table class="table tabe-hover table-bordered table-sm text-sm text-break" id="list">
            ...
            <tbody>
                <?php
                    $query = $conn->query("SELECT * FROM rider_payout order by created_at DESC ");
                    $index = 0;
...
//     _conf("Are you sure to Confirm this parcel?", "confirm_payout", [$(this).attr('id')])
// })
...
function confirm_payout($id) {
    start_load()
    $.ajax({
        url: 'ajax.php?action=rider_confirm_payout',
        method: 'POST',
        data: {
            id: $id
        },
...
function cancel_payout($id) {
    start_load()
    $.ajax({
        url: 'ajax.php?action=rider_cancel_payout',
        method: 'POST',
        data: {
            id: $id
        },
