...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(isset($_POST['parcelref']) && !empty($_POST['payout_method'])){
        $ref = $_POST['parcelref'];
        $sql = "select * from rider_payout where parcelref='$ref' ";
        ...
        $sql = "INSERT INTO rider_payout set $data";
        $result = mysqli_query($conn, $sql);
        if ($result) {
...
<div class="modal fade mx-auto" id="payout" tabindex="-1" role="dialog" aria-labelledby="editModalLabel"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form class=" mx-auto p-5" action="index.php?page=rider_payout_request" method="post">
                ...
                <input type="hidden" name="rider_id" id="rider_id" value="<?php echo $_SESSION['login_id']?>">
        ...
        <div class="card-body">
            ...
            <table class="table tabe-hover table-bordered table-sm text-break text-sm text-center" id="list">
                ...
                <tbody>
                    ...
                    $qry = $conn->query("SELECT * from parcels where riderid = $logged_in_user_id and status = 9 order by date_created DESC"); 
                    ...
                    <tr class="text-break">
                        ...
                        <td><b><?php echo $row['cod']+$row['price']+$row['delivery_cost'] ?></b></td>
                        <td class="text-center">
                            <div>
                                <button onclick="payoutbtn(this.id)" type="button"
                                    class="btn btn-sm btn-success btn-flat" id="<?php echo $index ?>">
...
function payoutbtn(id) {
    ...
    amount.value = parseInt(table.cells[4].innerText);
    request_amount.innerText = "Your Request Amount: " + table.cells[4].innerText;
